...
{
	return FoxHtmlElem::create('form')->attr('name', "fox-form-{$uid}")->attr('action', JRoute::_('index.php', false))->attr('method', 'post')->attr('enctype', 'multipart/form-data')->classes("fox-form fox-form-{$form->getDesign()->get('option.form.render')} fox-form-label-{$form->getDesign()->get('option.label.position')}")->append("<!-- Fox Contact [scope:{$form->getScope()}] [id:{$form->getId()}] [ver:{$form->getVersion()}] [time:" . date('i.s') . '] -->')->append(FoxFormRender::renders('row', $form->getDesign()->get('rows')))->append(FoxHtmlElem::create('input')->attr('type', 'hidden')->attr('name', 'option')->attr('value', 'com_foxcontact'))->append(FoxHtmlElem::create('input')->attr('type', 'hidden')->attr('name', 'task')->attr('value', 'form.send'))->append(FoxHtmlElem::create('input')->attr('type', 'hidden')->attr('name', 'uid')->attr('value', $uid))->append(FoxHtmlElem::create('input')->attr('type', 'hidden')->attr('name', 'fox_form_page_uri')->attr('value', JUri::getInstance()))->append(FoxHtmlElem::create('input')->attr('type', 'hidden')->attr('name', 'fox_form_page_title')->attr('value', JFactory::getDocument()->getTitle()));
}))->show();